...
begin
  ...
  res := FT_OpenEx(pchar(deviceDescription), FT_OPEN_BY_DESCRIPTION, @hnd);
...
begin
  ...
  { Try to open the device by description }
  res := FT_OpenEx(pchar(deviceDescription), FT_OPEN_BY_DESCRIPTION, @self.fhandle);
...
begin
  ...
  serialnumber := strpas(pchar(@s1[0]));
  description := strpas(pchar(@s2[0]));
...
begin
  ...
  res := FT_OpenEx(pchar(deviceSerial), FT_OPEN_BY_SERIAL_NUMBER, @hnd);
...
begin
  ...
  { Try to open the device by serial }
  res := FT_OpenEx(pchar(deviceSerial), FT_OPEN_BY_SERIAL_NUMBER, @self.fhandle);
